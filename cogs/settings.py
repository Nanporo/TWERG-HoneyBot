...
class SettingsView(discord.ui.View):
    ...
    async def excluded_callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"✅ 已更新排除防護的身份組：\n{role_mentions if role_mentions else '無 (僅保留預設排除)'}", ephemeral=True)
    ...
    async def trap_callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"✅ 已更新陷阱身份組：\n{role_mentions if role_mentions else '無'}", ephemeral=True)
    ...
    async def toggle_callback(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(f"✅ 封鎖時刪除近30分鐘訊息功能已 **{'啟用' if new_val else '停用'}**", ephemeral=True)
    ...
    async def console_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"❌ 更新設定發生錯誤: {e}", ephemeral=True)
