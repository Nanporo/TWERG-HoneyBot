...
class FkfeboyCog(commands.Cog):
    ...
    def get_settings(self):
        ...
        若 JSON 已存在，自動升級並合併新的預設關鍵字與目標用戶。
        """
        default_settings = {
            "target_users": [
                964849855396741130,
                ...
                782499307717656596, 
                815574915901554699, # ExpTech 管理員
                69370157784371200, 
            ],
            "bad_words": [
                # 1. 傳統粗口、公然侮辱與威脅
                "幹破", "放炸彈", "破狗", "破草", "王八蛋", "好好跟你說", "主機板", 
                "操你媽的", "操你媽", "幹你娘", "幹妳娘", "炸你", "死賤貨", "賤貨", "殺小", 
                "雞巴", "炸群", "鬼態度", "好好講",

                # 2. 人身攻擊、智力/身障侮辱與詛咒 (幹男愛用)
                "低能兒", "低能", "智障", "弱智", "腦殘", "腦癱", "賤畜", "死狗", 
                "欠壓S", "欠壓", "壓S", "欠幹", "欠操", "死全家", "全家死光", "家也會被震垮", 
                "震垮", "憨點", "憨包",

                # 3. 政治仇恨攻擊與侮辱性稱呼 (藍白草/白藍狗/極端政治用語)
                "白藍狗", "藍白狗", "藍白草", "白藍草", "藍草", "白草", "小草", "草包", "雜草",
                "藍白賤畜", "白藍賤畜", "死藍白", "死白藍", "藍狗", "白狗", "藍豬", "白豬", 
                "柯粉", "蔥粉", "黃狗",

                # 4. 假地震恐嚇、造謠與騷擾預測標籤 (避免過度廣泛，精準設定極端恐嚇詞彙)
                "毀滅性", "7級巨震", "巨震", "大噴發", "預報員", "預測員", "預側", 
                "預測文", "雙北毀滅", "開香檳慶祝", "靈氣強震", "3主震", "選我正解"
            ]
        }
    ...
    async def on_message(self, message: discord.Message):
        ...
        content_has_bad_word = any(word in content_to_check for word in bad_words)
        name_has_bad_word = any(word in display_name for word in bad_words)
        has_bad_word = content_has_bad_word or name_has_bad_word

        # 3. 懲罰規則：
        # 條件一：同時 at 特定保護對象 2 人以上 -> 直接 BAN
        # 條件二：at 特定保護對象 1 人以上，且發言或暱稱包含違規/攻擊關鍵字 -> 直接 BAN
        if mentioned_target_count >= 2 or (mentioned_target_count >= 1 and has_bad_word):
            try:
                reason = "觸發幹婆你男娘防禦：惡意標記特定保護用戶"
                await message.author.ban(reason=reason, delete_message_seconds=1800)
                print(f"🚨 [幹男防護] 已 Ban 惡意用戶 {message.author} ({message.author.id}) - 理由: 惡意標記與攻擊")
        ...
        # 條件三：發言或暱稱包含違規/攻擊關鍵字（未 at 目標用戶） -> Kick 並嘗試刪除訊息
        if has_bad_word:
            # 嘗試刪除該筆違規訊息
            try:
            ...
            try:
                reason = "觸發幹婆你男娘防禦：發布恐嚇、政治仇恨或不當字眼"
                await message.author.kick(reason=reason)
                print(f"🚨 [幹男防護] 已 Kick 惡意用戶 {message.author} ({message.author.id}) - 理由: 恐嚇/不當言論或暱稱")
            except discord.Forbidden:
