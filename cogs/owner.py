...
import os
import json
from cogs.server_check import load_server_authorizations, save_server_authorizations
...
class Owner(commands.Cog):
    ...
    @app_commands.command(name="推送", description="（限擁有者）強制推送訊息到指定頻道")
    ...
    @app_commands.command(name="關機", description="（限擁有者）關閉機器人")
    @app_commands.guilds(518699949500661760, 897116721159233576)
    @app_commands.check(is_owner)
    ...
    @app_commands.command(name="授權", description="（限擁有者）核准或撤銷伺服器的機器人使用權限")
    ...
    @app_commands.command(name="信任管理", description="（限擁有者）新增、刪除或查看授權操作 Ryker 小帳清單的信任人員")
