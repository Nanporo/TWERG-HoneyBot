    ...
    if message.author.id in bad_users_set:
        ...
        if bot_member.guild_permissions.moderate_members and bot_member.top_role > message.author.top_role:
            try:
                ...
                await message.author.timeout(duration, reason="匹配至惡意破壞者黑名單帳號 (bad_users)")
                await cog._send_kill_announcement(message.channel, message.author, "匹配至惡意破壞者黑名單帳號", message.content)
